...
export const DashboardPage = () => {
  ...
  const getAIRecommendations = async () => {
    ...
    try {
      ...
      const completedTasks = user.developmentTasks.filter(
        (t: any) => t.completed
      ).length;
      const userState = {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (roadmap.currentMonth) {
          roadmap.currentMonth.daily?.forEach((task: any) =>
            allTasks.push({ ...task, completed: false })
          );
          roadmap.currentMonth.weekly?.forEach((task: any) =>
            allTasks.push({ ...task, completed: false })
          );
          roadmap.currentMonth.monthly?.forEach((task: any) =>
            allTasks.push({ ...task, completed: false })
          );
        }
  ...
  useEffect(() => {
    ...
    if (
      ...
      (!user.selectedCareer || user.role === "Career Explorer")
    ) {
      fetchCareerTracks();
    }
          ...
          <div className="flex flex-wrap justify-between gap-3 p-4 animate-fade-in">
            <div className="flex min-w-72 flex-col gap-2">
              ...
              <p className="text-muted-foreground text-base font-normal leading-normal">
                {user.major
                  ? `Your personalized ${user.major} career dashboard`
                ...
                <div className="flex flex-wrap items-center gap-3 mt-3">
                  ...
                  {user.experience && (
                    <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-purple-500/10 border border-purple-500/20">
                      ...
                      <span className="text-xs font-medium text-purple-700 dark:text-purple-400 capitalize">
                        {user.experience}
                      </span>
                    </div>
                  )}
                ...
                careerTracks.length > 0 && (
                  <Card className="animate-fade-in bg-gradient-to-br from-primary/10 to-purple-500/10 border-primary/20">
                    <CardHeader>
                      ...
                      <CardDescription>
                        Based on your {user.major} major and skills, here are
                        career paths we recommend
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                ...
                <Card className="animate-fade-in">
                  <CardContent className="p-8 text-center">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Finding the perfect career tracks for you...
                    </p>
                  </CardContent>
                </Card>
              )}
                ...
                <Link href="https://ai-technical-interviewer-201743667943.us-west1.run.app">
                  <Card className="cursor-pointer hover:scale-105 hover:shadow-xl transition-all bg-gradient-to-br from-purple-500 to-pink-500 text-primary-foreground border-0 h-full">
                    <CardContent className="p-6 space-y-2">
                      ...
                      <p className="text-sm opacity-90">
                        {user.role
                          ? `Practice for ${user.role.replace(
                              "Aspiring ",
                              ""
                            )} roles`
                          : "Practice interviews"}
              ...
              {aiSuggestions && (
                <Card className="animate-fade-in bg-gradient-to-br from-primary/10 to-purple-500/10 border-primary/20">
                  ...
                  <CardContent className="space-y-4">
                    ...
                    {aiSuggestions.urgentActions &&
                      aiSuggestions.urgentActions.length > 0 && (
                        <div className="space-y-2">
                          ...
                          <ul className="text-sm space-y-1 list-disc pl-5 text-muted-foreground">
                            {aiSuggestions.urgentActions.map(
                    ...
                    {aiSuggestions.recommendedActions && (
                      <div className="space-y-2">
                        ...
                        <ul className="text-sm space-y-1 list-disc pl-5 text-muted-foreground">
                          {aiSuggestions.recommendedActions.map(
                            (action: string, i: number) => (
                              <li key={i}>{action}</li>
                            )
                          )}
                ...
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <div>
                    ...
                    <CardDescription className="mt-1">
                      {user.careerGoals
                        ? `Working towards: ${user.careerGoals.substring(
                            0,
                            50
                          )}...`
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    <div className="flex justify-between items-center">
                      ...
                      <p className="text-sm font-bold text-primary">
                        {user.developmentProgress}%
                      </p>
                    </div>
                    <Progress
                      value={user.developmentProgress}
                      className="h-2.5"
                    />
                  </div>
                  ...
                  {user.onboardingCompleted && (
                    <div className="pt-2 border-t">
                      <p className="text-xs text-muted-foreground">
                        💡 Click the sparkle icon to regenerate AI-powered tasks
                ...
                <>
                  ...
                  <div>
                    ...
                    <ul className="space-y-2">
                      {careerDetails.keyResponsibilities?.map(
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {careerDetails.requiredSkills?.map(
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      <div className="p-3 bg-muted rounded-lg">
                        ...
                        <p className="font-semibold">
                          {careerDetails.averageSalary?.entry || "N/A"}
                        </p>
                      </div>
                      <div className="p-3 bg-muted rounded-lg">
                        <p className="text-xs text-muted-foreground mb-1">
                        ...
                        <p className="font-semibold">
                          {careerDetails.averageSalary?.mid || "N/A"}
                        </p>
                      </div>
                      <div className="p-3 bg-muted rounded-lg">
                        <p className="text-xs text-muted-foreground mb-1">
                        ...
                        <p className="font-semibold">
                          {careerDetails.averageSalary?.senior || "N/A"}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <ul className="space-y-2">
                      {careerDetails.careerOpportunities?.map(
